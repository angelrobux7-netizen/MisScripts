...
-- Primer sonido eliminado
	...
	task.spawn(function()
		local v5 = tick()
		while tick() - v5 < u1 do
			imageLabel.Rotation = imageLabel.Rotation + u2
			task.wait(0.01)
		end
	end)
	task.spawn(function()
		local v6 = tick()
		while tick() - v6 < u3 do
			local v7 = (tick() - v6) / u3 * 1.2
			imageLabel.Size = UDim2.new(v7, 0, v7, 0)
			if u4 <= v7 then
				local v8 = (v7 - u4) / (1.2 - u4)
				imageLabel.ImageTransparency = math.clamp(v8, 0, 1)
			end
			task.wait(0.01)
		end
	...
	frame.Size = UDim2.new(1, 0, 1, 0)
		frame.BackgroundColor3 = Color3.fromRGB(120, 0, 255) -- Morado vibrante
		frame.BackgroundTransparency = 0.4
		frame.ZIndex = 0
		local uIGradient = Instance.new('UIGradient')
		uIGradient.Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, Color3.fromRGB(150, 50, 255)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(80, 0, 180)), ColorSequenceKeypoint.new(1, Color3.fromRGB(150, 50, 255)) })
	...
	imageLabel2.Image = 'rbxassetid://13879097036'
	imageLabel2.ScaleType = Enum.ScaleType.Fit
	imageLabel2.BorderSizePixel = 0
	imageLabel2.ImageTransparency = 0
		local sound2 = Instance.new('Sound')
			sound2.SoundId = 'rbxassetid://76650356472656' -- Buena Vida Mala Fama
			sound2.Volume = 2
			sound2.TimePosition = 3 -- Empieza en el segundo 3
			sound2.Parent = playerGui
			sound2:Play()
			
			-- Detener la música a los 11 segundos (después de 8 segundos de sonar)
	...
	task.spawn(function()
		local v15 = tick()
		while imageLabel2.Parent do
			if tick() - v15 >= u9 - u14 then
				u11 = math.max(u11 - u13, 0)
			else
				u11 = math.min(u11 + u13, u12)
			end
			imageLabel2.Rotation = imageLabel2.Rotation + u11
			task.wait(0.01)
		end
	end)
	task.spawn(function()
		local v16 = tick()
		while tick() - v16 < u10 do
			local v17 = (tick() - v16) / u10
			imageLabel2.Size = UDim2.new(v17, 0, v17, 0)
			task.wait(0.01)
		end
	...
	task.spawn(function()
		...
		local u142 = u14
		local v18 = tick()
		while tick() - v18 < u142 do
			local v19 = (tick() - v18) / u142
			imageLabel2.ImageTransparency = v19
			frame.BackgroundTransparency = 0.4 + v19 * 0.6
			uIGradient.Transparency = NumberSequence.new(v19)
			task.wait(0.01)
		end
	end)
	task.delay(u9, function()
		-- Animación de letras NIGHT DUEL
		local textLabel = Instance.new("TextLabel")
		textLabel.Parent = screenGui2
		textLabel.Size = UDim2.new(1, 0, 1, 0)
		textLabel.BackgroundTransparency = 1
		textLabel.Text = "NIGHT DUEL"
			textLabel.TextColor3 = Color3.fromRGB(170, 0, 255) -- Morado letras
			textLabel.Font = Enum.Font.GothamBlack
		textLabel.TextSize = 1
		textLabel.TextTransparency = 1
		
		-- Borde negro (UIStroke)
		...
		local tweenService = game:GetService("TweenService")
		tweenService:Create(textLabel, TweenInfo.new(1.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
			TextSize = 80,
			TextTransparency = 0
		}):Play()
		tweenService:Create(uiStroke, TweenInfo.new(1.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
			Transparency = 0
		}):Play()
		
		task.wait(2.5)
		
		-- Desvanecer letras y cerrar
		tweenService:Create(textLabel, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
			TextTransparency = 1,
			TextSize = 120
		}):Play()
		tweenService:Create(uiStroke, TweenInfo.new(1, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
			Transparency = 1
		}):Play()
...
local State = {
	...
	local function startAutoSteal()
		...
		Conns.autoSteal = RunService.Heartbeat:Connect(function()
			if not State.autoGrabEnabled or State.isStealing then return end
...
local function makePillToggle(parent, defOn, onToggle)
	...
	local function setV(on)
		isOn = on
		TweenService:Create(pbg, TweenInfo.new(0.18), {BackgroundColor3=on and Color3.fromRGB(0, 255, 255) or DIM2}):Play()
		TweenService:Create(ps,  TweenInfo.new(0.18), {Color=on and Color3.fromRGB(0, 255, 255) or BORDER2}):Play()
...
local function rowActionBtn(tabName, label, onClick)
	...
	b.MouseButton1Click:Connect(function()
		TweenService:Create(b, TweenInfo.new(0.08), {BackgroundColor3=Color3.fromRGB(0, 255, 255), TextColor3=Color3.fromRGB(0,0,0)}):Play()
		task.delay(0.15, function() TweenService:Create(b, TweenInfo.new(0.1), {BackgroundColor3=CARD_BG, TextColor3=WHITE}):Play() end)
...
do
	...
	local Q_OFF      = CARD_BG 
	local Q_ON       = Color3.fromRGB(0, 255, 255) -- COLOR CIAN: Se pone cian al presionar/encender
	local Q_TEXT_OFF = WHITE 
	local Q_TEXT_ON  = Color3.fromRGB(0, 0, 0) -- Texto negro sobre cian para legibilidad
	...
	createMobileButton("Drop", "DROP\nBR", 0, 0, false, function() task.spawn(runDrop) end)\n\t\tlocal _, saAG = createMobileButton("AutoGrab", "AUTO\nGRAB", 2, 0, true, function(on)\n\t\t\tState.autoGrabEnabled = on\n\t\t\tif on then startAutoSteal() else stopAutoSteal() end\n\t\tend)\n\t\tsetAutoGrab = function(on) saAG(on); State.autoGrabEnabled = on; if on then startAutoSteal() else stopAutoSteal() end end
...
startBatAimbotV2 = function()
	    ...
	    State.autoBatV2Enabled = true
	    State.autoSwingEnabled = true -- Aseguramos que el auto-swing esté activo con el bypass
    ...
    Conns.aimbotV2 = RunService.Heartbeat:Connect(function(dt)
        ...
        if target then
            ...
            if targetVel.Magnitude > 130 then targetVel = targetVel.Unit * 28 end
	            ...
	            -- PREDICCIÓN ADAPTATIVA ULTRA-PRECISA
	            local ping = 0.05 -- Estimación de ping base
	            local predictionFactor = math.clamp((distance / 160) + ping, 0.02, 0.18)
	            
	            -- Si el objetivo está muy cerca, reducimos la predicción para evitar "over-shooting"
	            if distance < 6 then 
	                predictionFactor = ping * 0.5
	            end 
	            
	            -- Base de la posición predictiva (Mejorada con aceleración si es posible)
	            local predictedTargetPos = target.Position + (targetVel * predictionFactor)
	            
	            -- POSICIONAMIENTO AGRESIVO OPTIMIZADO: Pegado detrás y ligeramente arriba para headshots
	            local targetLookDir = target.CFrame.LookVector
	            local standPos = predictedTargetPos - (targetLookDir * 0.8) + Vector3.new(0, 1.5, 0)
            ...
            local hDir = Vector3.new(moveDir.X, 0, moveDir.Z)
	            ...
	            -- Velocidad horizontal aumentada para mejor pegado (62 studs)
	            local hVel = Vector3.zero
	            if hDir.Magnitude > 0.15 then
	                hVel = hDir.Unit * 62  
	            end
	            
	            -- Velocidad vertical optimizada para estabilidad aérea
	            local vVelHeight = moveDir.Y * 35
	            local vVel = Vector3.new(0, math.clamp(vVelHeight, -85, 85), 0)
	            
	            -- Interpolación física lineal (Lerp) optimizada para máxima fluidez
	            local finalVel = hVel + vVel
	            root.AssemblyLinearVelocity = root.AssemblyLinearVelocity:Lerp(finalVel, math.min(dt * 35, 0.95))
	        ...
	        -- Auto-Swing Bypass (Velocidad de ataque optimizada y detección de remotos)
	        if State.autoSwingEnabled or State.autoBatV2Enabled then
	            local bat = char:FindFirstChildOfClass("Tool")
	            if bat and (bat.Name:lower():find("bat") or bat.Name:lower():find("slap")) then
	                local now = tick()
	                if not _lastV2Swing or now - _lastV2Swing >= 0.12 then -- Velocidad de swing máxima segura
	                    _lastV2Swing = now
...
stopBatAimbotV2 = function()
	    State.autoBatV2Enabled = false
	    State.autoSwingEnabled = false -- Desactivamos el auto-swing al detener el bypass
	    if Conns.aimbotV2 then Conns.aimbotV2:Disconnect(); Conns.aimbotV2 = nil end
    local c = LP.Character
    local root = c and c:FindFirstChild("HumanoidRootPart")
    local hum2 = c and c:FindFirstChildOfClass("Humanoid")
...
loadConfig = function()
    ...
    if cfg.antiLag           then State.antiLagEnabled=true;           if setAntiLag           then setAntiLag(true)           end end\n\tif cfg.autoGrabEnabled   then State.autoGrabEnabled=true;          if setAutoGrab          then setAutoGrab(true)          end end
